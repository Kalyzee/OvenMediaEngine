...
bool TransportCc::AddPacketFeedbackInfo(const std::shared_ptr<PacketFeedbackInfo>& packet_feedback_info)
{
	...
	uint16_t index = wide_sequence_number >= _base_sequence_number ? wide_sequence_number - _base_sequence_number : 0xFFFF - _base_sequence_number + wide_sequence_number + 1;
	if (index > 0x1000)
	{
	...
	while (index > _packet_feedbacks.size())
	{
		_packet_feedbacks.push_back(std::make_shared<PacketFeedbackInfo>(_base_sequence_number + _packet_feedbacks.size(), false));
		_packet_status_count++;
	}

	if (index == _packet_feedbacks.size())
	{
...
bool TransportCc::CalculeDeltas()
{
	std::shared_ptr<TransportCc::PacketFeedbackInfo> last_packet = nullptr;
	for (uint32_t i = 0; i < _packet_feedbacks.size(); i++)
	{
		...
		if (info->_received)
		{
			...
			{
				delta_us = info->_received_time_us - last_packet->_received_time_us;
			}
			last_packet = info;
		}
